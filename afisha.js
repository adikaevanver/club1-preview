...
(function () {
  ...
  function initAfisha(root){
    ...
    /* Какие дни сейчас на экране. Возвращаем СПИСОК, а не одну дату: в
       ...
       нет (правка Анвера 19.08).

       Горит день, чья карточка видна ЦЕЛИКОМ (правка Анвера 20.08). Раньше
       день засчитывался по любому пересечению с окном: от ряда сверху была
       видна одна строка с ценой, от ряда снизу — макушки афиш, а в рейке
       всё равно горели все их числа. На его примере светились 20–29, тогда
       как целиком читался только средний ряд. Теперь кромка сверху — низ
       липкой рейки, снизу — низ окна, и в счёт идут карточки, помещающиеся
       между ними полностью.

       Частично видимые оставлены запасным вариантом: если карточка выше
       доступной высоты (маленький экран, длинное название в две строки),
       целиком она не поместится ни при какой прокрутке, и рейка иначе
       погасла бы совсем. */
    function daysInView(){
      ...
      for (var i = 0; i < cards.length; i++){
        ...
        if (!d) continue;
        if (r.top >= line && r.bottom <= vh){
          if (full.indexOf(d) === -1) full.push(d);
        } else if (part.indexOf(d) === -1){
          part.push(d);
        }
      }
      return full.length ? full : part;
    }
    ...
    /* Смена ШИРИНЫ окна меняет число колонок — пока «Загрузить ещё» не
       ...
       Сверяем именно ширину (правка Анвера 20.08, «блики»). На телефоне
       прокрутка прячет и показывает адресную строку Safari, окно меняет
       высоту, и на каждое такое движение прилетает resize. Прежний
       обработчик перерисовывал ленту на любой resize, а render() собирает
       её через innerHTML — все карточки уничтожались и создавались заново,
       афиши гасли до подложки и декодировались с нуля. Отсюда и брались
       вспышки при прокрутке, и они же догоняли экран уже после остановки:
       дебаунс 150 мс срабатывал, когда палец уже убран.

       Замер на вьюпорте 402×874: после resize, менявшего только высоту,
       из 23 узлов афиш не выживал ни один. Высота на число колонок не
       влияет, поэтому такие resize пропускаем. */
    var loadedMore = false, resizeT = null, lastWidth = window.innerWidth;
